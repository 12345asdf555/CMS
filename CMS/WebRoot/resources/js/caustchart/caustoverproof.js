...
var charts,Series = [];
function showCaustOverproofChart(num){
	if(num==0){
	   	//初始化echart实例
...
function CaustHourDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) { 
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='itemChart/goItemoverproof?parent="+result.arys1[m].itemid+"&parentime1="+dtoTime1+"&parentime2="+dtoTime2+"'>"+result.arys1[m].name+"(min)</a>",width:width,halign : "center",align : "center"});
	 ...
	 $("#caustOverproofTable").datagrid( {
			fitColumns : true,
			height : $("#bodydiv").height() - $("#caustOverproofChart").height()-$("#caustOverProof_btn").height()-45,
			width : $("#bodydiv").width(),
			idField : 'id',
			pageSize : 10,
