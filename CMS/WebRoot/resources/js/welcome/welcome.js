...
function workRankDatagrid(id){
	$("#workRankTable").datagrid( {
		...
		url : "hierarchy/getWorkRank?parent="+id,
		singleSelect : true,
		columns : [ [ {
			field : 'rownum',
			title : '排名',
...
function useRatio(id){
	$.ajax({
		type : 'post',
		asyn : false,
		url : 'hierarchy/getUseRatio?parent='+id,
		dataType : 'json',
		success : function(result){
...
function loadRate(id){
	$.ajax({
		type : 'post',
		asyn : false,
		url : 'hierarchy/getLoadRate?parent='+id+'&time1='+$("#dtoTime1").combobox('getValue')+'&time2='+$("#dtoTime2").combobox('getValue'),
		dataType : 'json',
		success : function(result){
