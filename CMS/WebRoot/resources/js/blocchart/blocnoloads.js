         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='companyChart/goCompanyNoLoads?parent="+result.arys1[m].itemid+"&parentime1="+dtoTime1+"&parentime2="+dtoTime2+"'>"+result.arys1[m].name+"(空载率)</a>",width:width,halign : "center",align : "center"});
...
//改变表格高宽
function domresize() {
	$("#blocNoLoadsTable").datagrid('resize', {
		height : $("#bodydiv").height() - $("#blocNoLoadsChart").height()-$("#blocNoLoads_btn").height()-15,
		width : $("#bodydiv").width()
	});
	echarts.init(document.getElementById('blocNoLoadsChart')).resize();
