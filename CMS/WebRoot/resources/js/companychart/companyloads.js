...
var Series = [];
function showCompanyLoadsChart(num){
	if(num==0){
	   	//初始化echart实例
...
function CompanyloadsDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='caustChart/goCaustLoads?parent="+result.arys1[m].itemid+"&parentime1="+dtoTime1+"&parentime2="+dtoTime2+"'>"+result.arys1[m].name+"(负荷率)</a>",width:width,halign : "center",align : "center"});
	 ...
	 $("#companyLoadsTable").datagrid( {
			fitColumns : true,
			height : $("#bodydiv").height() - $("#companyLoadsChart").height()-$("#companyLoads_btn").height()-70,
			width : $("#bodydiv").width(),
			idField : 'id',
			pageSize : 10,
