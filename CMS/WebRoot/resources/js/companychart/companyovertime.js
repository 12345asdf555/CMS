...
function CompanytimeDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='caustChart/goCaustOvertime?parent="+result.arys1[m].itemid+"&parentime1="+dtoTime1+"&parentime2="+dtoTime2+"'>"+result.arys1[m].name+"(台)</a>",width:width,halign : "center",align : "left"});
	 ...
	 $("#companyOvertimeTable").datagrid( {
			fitColumns : true,
			height : $("#body").height() - $("#companyOvertimeChart").height()-$("#companyOvertime_btn").height()-45,
			width : $("#body").width(),
			idField : 'id',
			pageSize : 10,
