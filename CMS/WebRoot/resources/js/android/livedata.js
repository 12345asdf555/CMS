		...
		if($("#ary").val()){
			...
			for(var i=0;i<machine.length;i++){
				for(var j=0;j<ary.length;j++){
					if(machine[i].fid == parseInt(ary[j].fid),10){
					...
					if(machine[f].fid==(parseInt(redata.substring(4+i, 8+i),10))){
						...
						case "00":
							...
							if(liveary.length==0){
								...
								$("#m4"+machine[f].fid).html(parseInt(redata.substring(12+i, 16+i),10)+"A");
								$("#m5"+machine[f].fid).html(parseFloat((parseInt(redata.substring(16+i, 20+i),10)/10).toFixed(2))+"V");
